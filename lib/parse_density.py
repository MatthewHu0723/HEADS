...
from lib.utils import cal_mean_std_bidensity
...
#####################################################################################################
def append_para_bidensity(data_objects, args):
	## Calculate the density in both input and output window, combining to train/val/test dataset ##
	max_dens_shift, avg_dens_shift, positive_shift_ratio = -1, 0, 0 # Statistical details

	train_para_list, val_para_list, test_para_list = [], [], []
	out_list = []
	if(args.dataset != 'ushcn'):
		split_ts = 1 - args.pred_window / (args.history + args.pred_window) # Timestamp spliting lookback window and forecast 
	else:
		split_ts = None

	for i in range(data_objects["n_train_batches"]):
		batch_dict = data_objects["train_dataloader"].__next__()
		extra_dict = cal_mean_std_bidensity(batch_dict, split_ts, args.alpha, args.ablation_mark)

		# Statical details calculation(optional)
		dens_shift = (extra_dict["regi_dens_y"] - extra_dict["regi_dens"]).flatten()
		max_dens_shift = max(max_dens_shift, torch.max(dens_shift).item())
		avg_dens_shift += torch.mean(dens_shift).item()
		positive_shift_ratio += len(dens_shift[dens_shift > 0]) / len(dens_shift)
		# Include density
		batch_dict.update(extra_dict)
		train_para_list.append(batch_dict)
	
	for i in range(data_objects["n_val_batches"]):
		batch_dict = data_objects["val_dataloader"].__next__()
		extra_dict = cal_mean_std_bidensity(batch_dict, split_ts, args.alpha, args.ablation_mark)
		...
		batch_dict.update(extra_dict)
		val_para_list.append(batch_dict)
	
	
	for i in range(data_objects["n_test_batches"]):
		batch_dict = data_objects["test_dataloader"].__next__()
		extra_dict = cal_mean_std_bidensity(batch_dict, split_ts, args.alpha, args.ablation_mark)
		...
		batch_dict.update(extra_dict)
		test_para_list.append(batch_dict)

	avg_dens_shift /= data_objects["n_train_batches"] + data_objects["n_val_batches"] + data_objects["n_test_batches"]
	positive_shift_ratio /= data_objects["n_train_batches"] + data_objects["n_val_batches"] + data_objects["n_test_batches"]
	out_list.append(train_para_list)
	out_list.append(val_para_list)
	out_list.append(test_para_list)
	return out_list, max_dens_shift, avg_dens_shift, positive_shift_ratio

def parse_density(args, patch_ts=False, length_stat=False, save_path=None, alpha=2, abl='0'):
	...
	if dataset_name in ["physionet", "mimic"]:
		...
		# data_min, data_max, time_max = get_data_min_max(total_dataset, device) # (n_dim,), (n_dim,)
	...
	args.alpha = alpha
	args.ablation_mark = abl
	data_list, max_dens_shift, avg_dens_shift, positive_shift_ratio = append_para_bidensity(data_objects, args)
	data_dict = {
		"data_list": data_list,
		"data_max": data_max,
		"data_min": data_min,
		"time_max": time_max,
		"max_dens_shift": max_dens_shift,
		"avg_dens_shift": avg_dens_shift,
		"pos_shift_ratio": positive_shift_ratio
	}
