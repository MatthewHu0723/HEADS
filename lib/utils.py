...
def check_cpu_tensors():
    for var_name, var_value in globals().items():
        if isinstance(var_value, torch.Tensor):
            print(f"变量名: {var_name}, 设备: {var_value.device}")
...
def calculate_s_nodata(t, q=2):
	...
	return s # 密度，稀疏度的倒数，范围在[0,1]之间

def regional_density(T, M, alpha, abl_mark):
		"""
		X: (L', )
		T: (L, ), 0 ~ 1, 升序
		M: (L, ), 0 or 1
		...
		T_Masked = T[M != 0]
		if(len(T_Masked) == 0): return torch.tensor(1e-5)
		if(abl_mark=='1'):
			Dx_r = torch.sum(M) / (M.numel() + 1e-5) + 1e-5
		elif(abl_mark=='2'):
			Dx_r = calculate_s_nodata(T_Masked, q=alpha) + 1e-5
		else:
			Dx_r = calculate_s_nodata(T_Masked, q=alpha) * torch.sum(M) / (M.numel() + 1e-5) + 1e-5

		
		return torch.max(Dx_r, torch.tensor(1e-5).to(T.device))
...
def calculate_s(x, t, p=2, q=2, switch=False):
	...
	if l_1_x == 0:
		...
		return torch.tensor(1e-8).to(x.device)
	# 计算 P(t)^q 并求和
	t = torch.tensor([t[i] if(i==0) else t[i] - t[i-1] for i in range(len(t))]).to(x.device)
	t = torch.tensor([0.5]).to(x.device) if len(t) == 0 else torch.cat((t, torch.tensor([0.5 - t[-1]]).to(x.device)))
	prob_t = t
	...
	if(switch):
		...
		s = (len(t) - 1) * (l_p_x / l_1_x) * (1 / (1 - q)) * torch.log(sum_prob_q)
...
def calculate_s_nodata(t, p=2, q=2):
	...
	# 计算 P(t)^q 并求和
	t = torch.tensor([t[i] if(i==0) else t[i] - t[i-1] for i in range(len(t))])
	t = torch.tensor([0.5]) if len(t) == 0 else torch.concat((t, torch.tensor([0.5 - t[-1]])))
	prob_t = t
	sum_prob_q = torch.sum(torch.tensor([ts ** q for ts in prob_t]))
...
def regional_density(X, T, M, output_tag=False, density_type=1):
	# X_Masked = X[M != 0]
	T_Masked = T[M != 0] - 0.5 if output_tag else T[M != 0]

	if(density_type==1):
		Dx_r = calculate_s(X, T_Masked)
	elif(density_type==2):
		Dx_r = calculate_s_nodata(T_Masked)
	else:
		Dx_r = torch.sum(M) / (M.numel() + 1e-8) + 1e-8
...
def density_calculation(Tx, M, alpha, abl_mark):
	# Regional Density Calculation
	density = torch.tensor([regional_density(Tx[i,:,j], M[i,:,j], alpha, abl_mark) for i in range(M.shape[0]) for j in range(M.shape[2])]).reshape(-1, M.shape[2]).to(M.device)
	
	return density

def cal_mean_std_bidensity(batch_dict, split_ts, alpha, abl_mark):
	sublist = {}
	B, F = batch_dict["observed_data"].shape[0], batch_dict["observed_data"].shape[-1]
	obs_tp = batch_dict["observed_tp"].unsqueeze(-1).repeat(1, 1, F) if len(batch_dict["observed_tp"].shape) < 3 else batch_dict["observed_tp"]
	pred_tp = batch_dict["tp_to_predict"].unsqueeze(-1).repeat(1, 1, F) if len(batch_dict["tp_to_predict"].shape) < 3 else batch_dict["tp_to_predict"]

	# Scaling timestamps to fit in tasks with different window lengths
	obs_mask, pred_mask = batch_dict["observed_mask"].reshape(B, -1, F), batch_dict["mask_predicted_data"]

	if(split_ts != None):
		in_ratio, out_ratio = 0.5 / split_ts, 0.5 / (1-split_ts) 
		scaled_obs_tp, scaled_pred_tp = obs_tp.reshape(B, -1, F), pred_tp.reshape(B, -1, F)
		scaled_obs_tp[obs_mask==1] *= in_ratio
		scaled_pred_tp[scaled_pred_tp>=split_ts] -= split_ts
		scaled_pred_tp *= out_ratio
	else:  # USHCN
		scaled_obs_tp, scaled_pred_tp = obs_tp.reshape(B, -1, F) / 2, pred_tp.reshape(B, -1, F) / 2
	# Calculate Density
	regi_dens = \
		density_calculation(scaled_obs_tp, obs_mask, alpha, abl_mark)
	regi_dens_y = \
		density_calculation(scaled_pred_tp, pred_mask, alpha, abl_mark)
	
	sublist["regi_dens"] = regi_dens.clone().detach()
	sublist["regi_dens_y"] = regi_dens_y.clone().detach()
