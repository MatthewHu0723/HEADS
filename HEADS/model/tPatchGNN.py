...
class tPatchGNN(nn.Module):
	...
	def IMTS_Model(self, x, mask_X):
		...
		x_patch = self.TTCN(x, mask_X) # (B*N*M, hid_dim-1) h^c_p
		...
		for layer in range(self.n_layer):
			...
			nodevec1 = self.nodevec1.view(1, 1, N, self.nodevec_dim).repeat(B, M, 1, 1) # 创建符合标准正态分布的随机参数
			nodevec2 = self.nodevec2.view(1, 1, self.nodevec_dim, N).repeat(B, M, 1, 1)
			x_gate1 = self.nodevec_gate1[layer](torch.cat([x, nodevec1.permute(0, 2, 1, 3)], dim=-1)) # 式(9) g
			x_gate2 = self.nodevec_gate2[layer](torch.cat([x, nodevec2.permute(0, 3, 1, 2)], dim=-1)) # k=2
			x_p1 = x_gate1 * self.nodevec_linear1[layer](x) # (B, M, N, 10)
			x_p2 = x_gate2 * self.nodevec_linear2[layer](x) # (B, M, N, 10)
			nodevec1 = nodevec1 + x_p1.permute(0,2,1,3) # (B, M, N, 10) E_(p,1)
	...
	def forecasting(self, batch_dict):
		time_steps_to_predict, X, truth_time_steps, mask, truth_mask, truth_val = \
		...
		B, M, L_in, N = X.shape
		L_out = time_steps_to_predict.shape[-1]
		self.batch_size = B
		...
		""" Decoder """
		if(len(time_steps_to_predict.shape) == 2): # (B, :)
			...
			time_steps_to_predict = time_steps_to_predict.reshape(B, -1, N)
			L_pred = time_steps_to_predict.shape[-2]
			time_steps_to_predict = time_steps_to_predict.view(B, -1, L_pred, 1)
		...
		return outputs, truth_val, truth_mask  # (1, B, Lp, N)